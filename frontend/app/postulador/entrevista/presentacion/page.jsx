...
export default function PresentacionEntrevista() {
  ...
  const startRecording = async () => {
    ...
    recorder.onstop = async () => {
      ...
      const duracion = Math.round((Date.now() - inicioGrabacion) / 1000); // ✅ SEGUNDOS
      await procesarAudio(audioBlob, duracion);
    };

    setInicioGrabacion(Date.now()); // ✅ IMPORTANTE: antes de .start()
    recorder.start();
  ...
  const procesarAudio = async (blob, tiempoRespuesta = 0) => {
    try {
      ...
      formData.append('tiempoRespuesta', tiempoRespuesta); // ✅ Enviando segundos reales
      ...
      if (gptTexto) {
        ...
        console.warn('No se recibió respuesta GPT');
      }
